...
public class JavaParserEnumDeclaration extends AbstractTypeDeclaration implements EnumDeclaration
{
	...
	@Override public List<ReferenceType> getAncestors()
	{
		...
		ReferenceType enumClass =
			ReflectionFactory.typeUsageFor(Enum.class, typeSolver).asReferenceType();
		TypeParameterDeclaration eTypeParameter =
			enumClass.getTypeDeclaration().getTypeParameters().get(0);
		enumClass = enumClass.deriveTypeParameters(
			new TypeParametersMap.Builder()
		...
		if (wrappedNode.getImplementedTypes() != null)
		{
			...
			{
				SymbolReference<TypeDeclaration> implementedDeclRef =
					new SymbolSolver(typeSolver)
						.solveTypeInType(this, implementedType.getName().getId());
				...
				ancestors.add(new ReferenceTypeImpl(
					(ReferenceTypeDeclaration)implementedDeclRef.getCorrespondingDeclaration(),
					typeSolver));
			}
		}
		return ancestors;
	...
	public static class ValuesMethod implements MethodDeclaration
	{
		...
		@Override public ReferenceTypeDeclaration declaringType()
		{
