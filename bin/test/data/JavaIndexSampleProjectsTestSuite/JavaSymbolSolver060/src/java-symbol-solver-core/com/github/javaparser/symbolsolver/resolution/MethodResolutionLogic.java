...
{
	private static List<Type> groupVariadicParamValues(
		List<Type> argumentsTypes, int startVariadic, Type variadicType)
	{
	...
	public static boolean isApplicable(
		MethodDeclaration method, String name, List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		...
		boolean withWildcardTolerance)
	{
					...
					if (!expectedType.isAssignableBy(actualType))
					{
						...
						else
						{
							argumentsTypes = groupVariadicParamValues(
								argumentsTypes, pos, method.getLastParam().getType());
						}
					}
		...
		for (int i = 0; i < method.getNumberOfParams(); i++)
		{
			...
			if (!isAssignableWithoutSubstitution)
			{
				...
				if (!expectedType.isAssignableBy(actualType))
				{
					if (actualType.isWildcard() && withWildcardTolerance &&
	...
	public static boolean isAssignableMatchTypeParameters(
		Type expected, Type actual, Map<String, Type> matchedParameters)
	{
	...
	public static boolean isAssignableMatchTypeParameters(
		ReferenceType expected, ReferenceType actual, Map<String, Type> matchedParameters)
	{
		...
		else
		{
			...
			{
				if (isAssignableMatchTypeParametersMatchingQName(
						expected, ancestor, matchedParameters))
				{
					return true;
				}
			}
		...
		ReferenceType expected, ReferenceType actual, Map<String, Type> matchedParameters)
	{
		...
		for (int i = 0; i < expected.typeParametersValues().size(); i++)
		{
			...
			else if (expectedParam.isWildcard())
			{
				...
				{
					return isAssignableMatchTypeParameters(
						expectedParam.asWildcard().getBoundedType(), actual, matchedParameters);
				}
			...
			if (type.describe().equals(tp.getName()))
			{
				...
				else
				{
					return new ReferenceTypeImpl(
						typeSolver.solveType(Object.class.getCanonicalName()), typeSolver);
				}
			}
			return type;
		}
		...
		else if (type.isReferenceType())
		{
			...
			result = result
						 .transformTypeParameters(
							 typeParam -> replaceTypeParam(typeParam, tp, typeSolver))
						 .asReferenceType();
			return result;
		}
	...
	public static boolean isApplicable(
		MethodUsage method, String name, List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		if (!method.getName().equals(name))
		{
			return false;
		...
		for (int i = 0; i < method.getNoParams(); i++)
		{
			...
			List<TypeParameterDeclaration> typeParameters =
				method.getDeclaration().getTypeParameters();
			typeParameters.addAll(method.declaringType().getTypeParameters());
			...
			for (Map.Entry<TypeParameterDeclaration, Type> entry: derivedValues.entrySet())
			{
				...
				expectedTypeWithInference = expectedTypeWithInference.replaceTypeVariables(
					tp, entry.getValue());
			}
			...
			for (TypeParameterDeclaration tp: typeParameters)
			{
				...
				{
					// expectedType = expectedType.replaceTypeVariables(tp.getName(), new
					// ReferenceTypeUsageImpl(typeSolver.solveType(Object.class.getCanonicalName()),
					// typeSolver));
					expectedType = expectedType.replaceTypeVariables(
						tp,
						Wildcard.extendsBound(new ReferenceTypeImpl(
							typeSolver.solveType(Object.class.getCanonicalName()), typeSolver)));
				}
				else if (tp.getBounds(typeSolver).size() == 1)
				{
					...
					if (bound.isExtends())
					{
						...
						expectedType = expectedType.replaceTypeVariables(
							tp, Wildcard.extendsBound(bound.getType()));
					}
					else
					{
						// expectedType = expectedType.replaceTypeVariables(tp.getName(), new
						...
						// typeSolver));
						expectedType = expectedType.replaceTypeVariables(
							tp, Wildcard.superBound(bound.getType()));
					}
				}
				else
				{
			...
			for (TypeParameterDeclaration tp: typeParameters)
			{
				if (tp.getBounds(typeSolver).isEmpty())
				{
					expectedType2 = expectedType2.replaceTypeVariables(
						tp,
	...
	private static List<MethodDeclaration> getMethodsWithoutDuplicates(List<MethodDeclaration> methods)
	{
		Set<MethodDeclaration> s = new TreeSet<MethodDeclaration>(new Comparator<MethodDeclaration>() {
			@Override public int compare(MethodDeclaration m1, MethodDeclaration m2)
			{
				if (m1 instanceof JavaParserMethodDeclaration &&
					m2 instanceof JavaParserMethodDeclaration &&
					((JavaParserMethodDeclaration)m1)
						.getWrappedNode()
						.equals(((JavaParserMethodDeclaration)m2).getWrappedNode()))
				{
					return 0;
				}
				return 1;
	...
	/**
	 * @param methods        we expect the methods to be ordered such that inherited methods are
	 *     later in the list
	 * @param name
	 ...
	 * @return
	 */
	public static SymbolReference<MethodDeclaration> findMostApplicable(
		List<MethodDeclaration> methods, String name, List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		SymbolReference<MethodDeclaration> res = findMostApplicable(
			methods, name, argumentsTypes, typeSolver, false);
		if (res.isSolved())
		{
			return res;
		}
	...
	{
		List<MethodDeclaration> applicableMethods =
			getMethodsWithoutDuplicates(methods)
				.stream()
		...
		else
		{
			...
			for (int i = 1; i < applicableMethods.size(); i++)
			{
				...
				{
					if (winningCandidate.declaringType().getQualifiedName().equals(
							other.declaringType().getQualifiedName()))
					{
						possibleAmbiguity = true;
					}
					else
					{
						// we expect the methods to be ordered such that inherited methods are later
						// in the list
					}
				}
			}
	...
	public static Optional<MethodUsage> findMostApplicableUsage(
		List<MethodUsage> methods, String name, List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		...
		else
		{
			...
			for (int i = 1; i < applicableMethods.size(); i++)
			{
				...
				else
				{
					...
					{
						// we expect the methods to be ordered such that inherited methods are later
						// in the list throw new UnsupportedOperationException();
					}
				}
			}
	...
	public static SymbolReference<MethodDeclaration> solveMethodInType(
		TypeDeclaration typeDeclaration, String name, List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		...
		TypeSolver typeSolver)
	{
		...
		if (typeDeclaration instanceof JavaParserEnumDeclaration)
		{
			...
			{
				return SymbolReference.solved(new JavaParserEnumDeclaration.ValuesMethod(
					(JavaParserEnumDeclaration)typeDeclaration, typeSolver));
			}
