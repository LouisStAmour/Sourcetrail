...
public class JavaParserClassDeclaration extends AbstractClassDeclaration
{
	...
	public JavaParserClassDeclaration(
		com.github.javaparser.ast.body.ClassOrInterfaceDeclaration wrappedNode, TypeSolver typeSolver)
	{
		...
		this.javaParserTypeAdapter = new JavaParserTypeAdapter<ClassOrInterfaceDeclaration>(
			wrappedNode, typeSolver);
	}
	...
	@Override public List<FieldDeclaration> getAllFields()
	{
		...
		for (BodyDeclaration<?> member: wrappedNode.getMembers())
		{
			...
			{
				com.github.javaparser.ast.body.FieldDeclaration field =
					(com.github.javaparser.ast.body.FieldDeclaration)member;
		...
		getInterfaces().forEach(
			interf -> interf.getTypeDeclaration().getAllFields().forEach(f -> { fields.add(f); }));
	...
	@Override public List<ConstructorDeclaration> getConstructors()
	{
		...
		for (BodyDeclaration<?> member: wrappedNode.getMembers())
		{
			if (member instanceof com.github.javaparser.ast.body.ConstructorDeclaration)
			{
				...
				declared.add(
					new JavaParserConstructorDeclaration(this, constructorDeclaration, typeSolver));
			}
		}
		if (declared.isEmpty())
		{
	...
	@Override public boolean hasDirectlyAnnotation(String canonicalName)
	{
		...
		{
			if (solveType(annotationExpr.getName().getId(), typeSolver)
	...
	@Override public boolean canBeAssignedTo(ReferenceTypeDeclaration other)
	{
		...
		if (this.wrappedNode.getImplementedTypes() != null)
		{
			...
			{
				ReferenceTypeDeclaration ancestor =
					(ReferenceTypeDeclaration) new SymbolSolver(typeSolver).solveType(type);
	...
	public SymbolReference<TypeDeclaration> solveType(String name, TypeSolver typeSolver)
	{
		...
		if (name.startsWith(prefix) && name.length() > prefix.length())
		{
			return new JavaParserClassDeclaration(this.wrappedNode, typeSolver)
				.solveType(name.substring(prefix.length()), typeSolver);
		}
	...
	{
		return this.wrappedNode.getTypeParameters()
			.stream()
			...
			if (member instanceof com.github.javaparser.ast.body.TypeDeclaration)
			{
				res.add(
					JavaParserFacade.get(typeSolver)
						.getTypeDeclaration((com.github.javaparser.ast.body.TypeDeclaration)member));
			}
		}
		return res;
	}
	...
	private ReferenceType toReferenceType(ClassOrInterfaceType classOrInterfaceType)
	{
		...
		{
			return new ReferenceTypeImpl(
				ref.getCorrespondingDeclaration().asReferenceType(), typeSolver);
		}
		List<Type> superClassTypeParameters =
			classOrInterfaceType.getTypeArguments()
				...
				.collect(Collectors.toList());
		return new ReferenceTypeImpl(
			ref.getCorrespondingDeclaration().asReferenceType(), superClassTypeParameters, typeSolver);
	}
}
