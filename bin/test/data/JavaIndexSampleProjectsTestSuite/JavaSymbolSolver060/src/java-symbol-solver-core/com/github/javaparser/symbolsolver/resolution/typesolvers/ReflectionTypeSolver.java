	...
	@Override public SymbolReference<ReferenceTypeDeclaration> tryToSolveType(String name)
	{
		if (!jreOnly || (name.startsWith("java.") || name.startsWith("javax.")))
		{
			try
			{
				...
				// Some implementations could return null when the class was loaded through the
				// bootstrap classloader see
				// https://docs.oracle.com/javase/8/docs/api/java/lang/Class.html#getClassLoader--
				if (classLoader == null)
				{
					throw new RuntimeException(
						"The ReflectionTypeSolver has been probably loaded through the bootstrap class loader. This usage is not supported by the JavaSymbolSolver");
				}
			...
			catch (ClassNotFoundException e)
			{
				...
				else
				{
					...
					if (parent.isSolved())
					{
						Optional<ReferenceTypeDeclaration> innerClass =
							parent.getCorrespondingDeclaration()
								.internalTypes()
								.stream()
