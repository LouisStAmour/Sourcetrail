...
{
	static Optional<MethodUsage> getMethodUsage(
		CtClass ctClass,
		String name,
		...
		for (CtMethod method: ctClass.getDeclaredMethods())
		{
			if (method.getName().equals(name))
			{
				...
				MethodUsage methodUsage = new MethodUsage(
					new JavassistMethodDeclaration(method, typeSolver));
				...
				try
				{
					...
					{
						SignatureAttribute.MethodSignature classSignature =
							SignatureAttribute.toMethodSignature(method.getGenericSignature());
						List<Type> parametersOfReturnType = parseTypeParameters(
							classSignature.getReturnType().toString(), typeSolver, invokationContext);
						...
						{
							newReturnType = newReturnType.asReferenceType().transformTypeParameters(
								tp -> parametersOfReturnType.remove(0));
						}
			...
			if (superClass != null)
			{
				Optional<MethodUsage> ref =
					new JavassistClassDeclaration(superClass, typeSolver)
						.solveMethodAsUsage(
							name, argumentsTypes, typeSolver, invokationContext, null);
				if (ref.isPresent())
				{
					return ref;
				}
	...
	private static List<Type> parseTypeParameters(
		String signature, TypeSolver typeSolver, Context invokationContext)
	{
	...
	static Type signatureTypeToType(
		...
		TypeParametrizable typeParametrizable)
	{
		if (signatureType instanceof SignatureAttribute.ClassType)
		{
			...
			List<Type> typeParameters = classType.getTypeArguments() == null
				? Collections.emptyList()
				: Arrays.stream(classType.getTypeArguments())
					  .map(ta -> typeArgumentToType(ta, typeSolver, typeParametrizable))
					  .collect(Collectors.toList());
			final String typeName = classType.getDeclaringClass() != null
		...
		else if (signatureType instanceof SignatureAttribute.TypeVariable)
		{
			...
			Optional<TypeParameterDeclaration> typeParameterDeclarationOpt =
			...
			if (!typeParameterDeclarationOpt.isPresent())
			{
				throw new UnsolvedSymbolException(
					"Unable to solve TypeVariable " + typeVariableSignature);
			}
	...
	private static Type objectTypeArgumentToType(
		...
		return type.orElseGet(
			()
				-> new ReferenceTypeImpl(
					typeSolver.solveType(internalNameToCanonicalName(typeName)), typeSolver));
	}

	private static Optional<Type> getGenericParameterByName(
		String typeName, TypeParametrizable typeParametrizable)
	{
		Optional<TypeParameterDeclaration> tp = typeParametrizable.findTypeParameter(typeName);
		return tp.map(TypeVariable::new);
	}

	private static Type typeArgumentToType(
		...
		TypeParametrizable typeParametrizable)
	{
		if (typeArgument.isWildcard())
		{
			...
			{
				return Wildcard.extendsBound(objectTypeArgumentToType(
					typeArgument.getType(), typeSolver, typeParametrizable));
			}
			else if (typeArgument.getKind() == '-')
			{
				return Wildcard.superBound(objectTypeArgumentToType(
					typeArgument.getType(), typeSolver, typeParametrizable));
			}
			else
			{
