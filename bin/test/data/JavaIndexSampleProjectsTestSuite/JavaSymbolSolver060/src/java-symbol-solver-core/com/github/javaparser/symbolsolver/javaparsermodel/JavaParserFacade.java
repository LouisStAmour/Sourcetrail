...
public class JavaParserFacade
{
	...
	protected static Type solveGenericTypes(Type type, Context context, TypeSolver typeSolver)
	{
		...
		else if (type.isWildcard())
		{
			if (type.asWildcard().isExtends() || type.asWildcard().isSuper())
			{
				...
				Type boundResolved = solveGenericTypes(
					wildcardUsage.getBoundedType(), context, typeSolver);
	...
	public SymbolReference<ConstructorDeclaration> solve(
		...
		ExplicitConstructorInvocationStmt explicitConstructorInvocationStmt, boolean solveLambdas)
	{
		...
		Optional<ClassOrInterfaceDeclaration> optAncestor =
			explicitConstructorInvocationStmt.getAncestorOfType(ClassOrInterfaceDeclaration.class);
		...
		{
			Type classDecl =
				JavaParserFacade.get(typeSolver).convert(classNode.getExtendedTypes(0), classNode);
		...
		{
			SymbolReference<TypeDeclaration> sr =
				JavaParserFactory.getContext(classNode, typeSolver)
					.solveType(classNode.getNameAsString(), typeSolver);
			if (sr.isSolved())
			{
		...
		SymbolReference<ConstructorDeclaration> res = ConstructorResolutionLogic.findMostApplicable(
			((ClassDeclaration)typeDecl).getConstructors(), argumentTypes, typeSolver);
		...
		ObjectCreationExpr objectCreationExpr, boolean solveLambdas)
	{
		...
		SymbolReference<ConstructorDeclaration> res = ConstructorResolutionLogic.findMostApplicable(
			((ClassDeclaration)classDecl.asReferenceType().getTypeDeclaration()).getConstructors(),
			argumentTypes,
			typeSolver);
		for (LambdaArgumentTypePlaceholder placeholder: placeholders)
		{
		...
		List<LambdaArgumentTypePlaceholder> placeholders)
	{
		...
		for (Expression parameterValue: args)
		{
			...
			else
			{
				...
				{
					argumentTypes.add(
						JavaParserFacade.get(typeSolver).getType(parameterValue, solveLambdas));
				}
	...
	public SymbolReference<AnnotationDeclaration> solve(AnnotationExpr annotationExpr)
	{
		...
		SymbolReference<TypeDeclaration> typeDeclarationSymbolReference = context.solveType(
			annotationExpr.getNameAsString(), typeSolver);
		AnnotationDeclaration annotationDeclaration =
			(AnnotationDeclaration)typeDeclarationSymbolReference.getCorrespondingDeclaration();
	...
	private Optional<Type> find(Map<Node, Type> map, LambdaExpr lambdaExpr)
	{
		for (Node key: map.keySet())
		{
			if (key instanceof LambdaExpr)
			{
				...
				if (keyLambdaExpr.toString().equals(lambdaExpr.toString()) &&
					getParentNode(keyLambdaExpr) == getParentNode(lambdaExpr))
				{
	...
	protected MethodUsage toMethodUsage(MethodReferenceExpr methodReferenceExpr)
	{
		...
		SymbolReference<TypeDeclaration> typeDeclarationSymbolReference =
			JavaParserFactory.getContext(classOrInterfaceType, typeSolver)
				.solveType(classOrInterfaceType.getName().getId(), typeSolver);
		if (!typeDeclarationSymbolReference.isSolved())
		{
			throw new UnsupportedOperationException();
		}
		List<MethodUsage> methodUsages =
			((ReferenceTypeDeclaration)typeDeclarationSymbolReference.getCorrespondingDeclaration())
				.getAllMethods()
				.stream()
				.filter(it -> it.getName().equals(methodReferenceExpr.getIdentifier()))
	...
	/**
	 * Should return more like a TypeApplication: a TypeDeclaration and possible
	 * typeParametersValues or array modifiers.
	 *
	 * @return
	 */
	...
	protected Type convertToUsage(com.github.javaparser.ast.type.Type type, Context context)
	{
		if (type instanceof ClassOrInterfaceType)
		{
			...
			{
				typeParameters = classOrInterfaceType.getTypeArguments()
									 .get()
									 .stream()
			...
			if (typeDeclaration.isTypeParameter())
			{
				...
				{
					JavaParserTypeVariableDeclaration javaParserTypeVariableDeclaration =
						(JavaParserTypeVariableDeclaration)typeDeclaration;
			...
			else
			{
				return new ReferenceTypeImpl(
					(ReferenceTypeDeclaration)typeDeclaration, typeParameters, typeSolver);
			}
		}
		else if (type instanceof com.github.javaparser.ast.type.PrimitiveType)
		{
			return PrimitiveType.byName(
				((com.github.javaparser.ast.type.PrimitiveType)type).getType().name());
		}
		else if (type instanceof WildcardType)
		{
			WildcardType wildcardType = (WildcardType)type;
			if (wildcardType.getExtendedTypes().isPresent() &&
				!wildcardType.getSuperTypes().isPresent())
			{
				return Wildcard.extendsBound(convertToUsage(
					wildcardType.getExtendedTypes().get(), context));	 // removed (ReferenceTypeImpl)
			}
			else if (!wildcardType.getExtendedTypes().isPresent() && wildcardType.getSuperTypes().isPresent())
			{
				return Wildcard.superBound(convertToUsage(
					wildcardType.getSuperTypes().get(), context));	  // removed (ReferenceTypeImpl)
			}
			else if (!wildcardType.getExtendedTypes().isPresent() && !wildcardType.getSuperTypes().isPresent())
			{
		...
		{
			com.github.javaparser.ast.type.ArrayType jpArrayType =
	...
	public MethodUsage solveMethodAsUsage(MethodCallExpr call)
	{
		...
		Optional<MethodUsage> methodUsage = context.solveMethodAsUsage(
			call.getName().getId(), params, typeSolver);
		if (!methodUsage.isPresent())
		{
			throw new RuntimeException(
				"Method '" + call.getName() + "' cannot be resolved in context " + call +
				" (line: " + call.getRange().get().begin.line + ") " + context +
				". Parameter types: " + params);
		}
		return methodUsage.get();
	}

	public ReferenceTypeDeclaration getTypeDeclaration(
	...
	public Type getTypeOfThisIn(Node node)
	{
		...
		{
			return new ReferenceTypeImpl(
				getTypeDeclaration((ClassOrInterfaceDeclaration)node), typeSolver);
		}
		else if (node instanceof EnumDeclaration)
		{
			JavaParserEnumDeclaration enumDeclaration = new JavaParserEnumDeclaration(
				(EnumDeclaration)node, typeSolver);
