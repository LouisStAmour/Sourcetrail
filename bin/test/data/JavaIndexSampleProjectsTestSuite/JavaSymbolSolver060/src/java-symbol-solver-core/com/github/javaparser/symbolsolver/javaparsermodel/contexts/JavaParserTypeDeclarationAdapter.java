...
public class JavaParserTypeDeclarationAdapter
{
				...
				if (internalType.getName().getId().equals(name))
				{
					return SymbolReference.solved(
						JavaParserFacade.get(typeSolver).getTypeDeclaration(internalType));
							 ...
							 String.format("%s.%s", wrappedNode.getName(), internalType.getName())))
				{
					return JavaParserFactory.getContext(internalType, typeSolver)
						.solveType(
							name.substring(wrappedNode.getName().getId().length() + 1), typeSolver);
				}
				else if (name.startsWith(String.format("%s.", internalType.getName())))
				{
					return JavaParserFactory.getContext(internalType, typeSolver)
						.solveType(
							name.substring(internalType.getName().getId().length() + 1), typeSolver);
				}
			}
		}
		...
		String name, List<Type> argumentsTypes, boolean staticOnly, TypeSolver typeSolver)
	{
		...
		{
			SymbolReference<MethodDeclaration> parentSolution = context.getParent().solveMethod(
		...
		List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		if (typeDeclaration instanceof ClassDeclaration)
		{
			return ConstructorResolutionLogic.findMostApplicable(
				((ClassDeclaration)typeDeclaration).getConstructors(), argumentsTypes, typeSolver);
		}
