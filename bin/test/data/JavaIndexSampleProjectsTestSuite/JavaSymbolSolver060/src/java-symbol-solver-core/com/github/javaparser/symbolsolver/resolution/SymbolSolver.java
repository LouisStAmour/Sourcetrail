...
public class SymbolSolver
{
	...
	{
		SymbolReference<MethodDeclaration> decl = context.solveMethod(
			methodName, argumentsTypes, false, typeSolver);
		if (!decl.isSolved())
		{
	...
	public TypeDeclaration solveType(com.github.javaparser.ast.type.Type type)
	{
		if (type instanceof ClassOrInterfaceType)
		{
			...
			SymbolReference<TypeDeclaration> ref =
				JavaParserFactory.getContext(type, typeSolver).solveType(name, typeSolver);
			if (!ref.isSolved())
			{
				throw new UnsolvedSymbolException(
					JavaParserFactory.getContext(type, typeSolver), name);
			}
	...
	/**
	 * Solve any possible visible symbols including: fields, internal types, type variables, the
	 * type itself or its containers. <p> It should contain its own private fields but not inherited
	 * private fields.
	 */
	public SymbolReference<? extends ValueDeclaration> solveSymbolInType(
