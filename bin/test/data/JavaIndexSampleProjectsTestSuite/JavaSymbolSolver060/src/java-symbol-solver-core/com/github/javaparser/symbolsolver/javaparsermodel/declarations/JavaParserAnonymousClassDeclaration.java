...
public class JavaParserAnonymousClassDeclaration extends AbstractClassDeclaration
{
	...
	public JavaParserAnonymousClassDeclaration(ObjectCreationExpr wrappedNode, TypeSolver typeSolver)
	{
		...
		superTypeDeclaration = JavaParserFactory
								   .getContext(wrappedNode.getParentNode().get(), typeSolver)
	...
	{
		return wrappedNode.getAnonymousClassBody()
			.get()
			.stream()
	...
	{
		return findMembersOfKind(com.github.javaparser.ast.body.ConstructorDeclaration.class)
			.stream()
	...
	{
		return ImmutableList.<ReferenceType>builder()
			.add(getSuperClass())
	...
	@Override public List<FieldDeclaration> getAllFields()
	{
		List<JavaParserFieldDeclaration> myFields =
			findMembersOfKind(com.github.javaparser.ast.body.FieldDeclaration.class)
				...
				.flatMap(
					field
					-> field.getVariables().stream().map(
						variable -> new JavaParserFieldDeclaration(variable, typeSolver)))
				.collect(Collectors.toList());

		List<FieldDeclaration> superClassFields = getSuperClass().getTypeDeclaration().getAllFields();
		...
		return ImmutableList.<FieldDeclaration>builder()
			.addAll(myFields)
	...
	{
		return findMembersOfKind(com.github.javaparser.ast.body.MethodDeclaration.class)
			.stream()
	...
	{
		return findMembersOfKind(com.github.javaparser.ast.body.TypeDeclaration.class)
			.stream()
