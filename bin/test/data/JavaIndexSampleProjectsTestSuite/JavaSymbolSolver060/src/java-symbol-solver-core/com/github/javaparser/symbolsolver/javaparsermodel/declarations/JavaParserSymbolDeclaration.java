...
public class JavaParserSymbolDeclaration implements ValueDeclaration
{
	...
	public static JavaParserSymbolDeclaration localVar(
		VariableDeclarator variableDeclarator, TypeSolver typeSolver)
	{
		return new JavaParserSymbolDeclaration(
			variableDeclarator, variableDeclarator.getName().getId(), typeSolver, false, false, true);
	}
	...
	@Override public String toString()
	{
		return "JavaParserSymbolDeclaration{"
			+ "name='" + name + '\'' + ", wrappedNode=" + wrappedNode + '}';
	}
	...
	@Override public Type getType()
	{
		if (wrappedNode instanceof Parameter)
		{
			...
			if (getParentNode(wrappedNode) instanceof LambdaExpr)
			{
				...
				// JavaParserFacade.get(typeSolver).getMethodCalled MethodDeclaration methodCalled =
				// JavaParserFacade.get(typeSolver).solve()
			...
			else
			{
				...
				{
					rawType = PrimitiveType.byName(
						((com.github.javaparser.ast.type.PrimitiveType)parameter.getType())
							.getType()
							.name());
				}
				else
				{
					rawType = JavaParserFacade.get(typeSolver)
								  .convertToUsage(parameter.getType(), wrappedNode);
				}
		...
		else if (wrappedNode instanceof VariableDeclarator)
		{
			...
			{
				Type type = JavaParserFacade.get(typeSolver)
								.convert(
	...
	@Override public TypeDeclaration asType()
	{
		throw new UnsupportedOperationException(
			this.getClass().getCanonicalName() + ": wrapping " +
			this.getWrappedNode().getClass().getCanonicalName());
	}
