...
public class JavassistClassDeclaration extends AbstractClassDeclaration
{
	...
	public JavassistClassDeclaration(CtClass ctClass, TypeSolver typeSolver)
	{
		...
		if (ctClass.isInterface() || ctClass.isAnnotation() || ctClass.isPrimitive() ||
			ctClass.isEnum())
		{
			throw new IllegalArgumentException(
		...
		this.javassistTypeDeclarationAdapter = new JavassistTypeDeclarationAdapter(
			ctClass, typeSolver);
	}

	@Override protected ReferenceType object()
	{
	...
	@Override public boolean equals(Object o)
	{
		...
		if (!ctClass.equals(that.ctClass))
			return false;
	...
	{
		return JavassistUtils.getMethodUsage(
			ctClass, name, argumentsTypes, typeSolver, invokationContext);
	}
	...
	public SymbolReference<? extends ValueDeclaration> solveSymbol(String name, TypeSolver typeSolver)
	{
		...
		try
		{
			...
			if (superClass != null)
			{
				SymbolReference<? extends ValueDeclaration> ref =
					new JavassistClassDeclaration(superClass, typeSolver).solveSymbol(name, typeSolver);
				if (ref.isSolved())
				{
					return ref;
				}
		...
		try
		{
			...
			{
				SymbolReference<? extends ValueDeclaration> ref = new JavassistInterfaceDeclaration(
																	  interfaze, typeSolver)
		...
		String name, List<Type> argumentsTypes, boolean staticOnly)
	{
		...
		for (CtMethod method: ctClass.getDeclaredMethods())
		{
			...
			if (method.getName().equals(name) && !isSynthetic && isNotBridge &&
	...
	@Override public boolean isAssignableBy(Type type)
	{
		...
		try
		{
			if (this.ctClass.getSuperclass() != null &&
				new JavassistClassDeclaration(this.ctClass.getSuperclass(), typeSolver)
					.isAssignableBy(type))
			{
				return true;
			}
	...
	@Override public ReferenceType getSuperClass()
	{
		try
		{
			...
			{
				return new ReferenceTypeImpl(
					new JavassistClassDeclaration(ctClass.getSuperclass(), typeSolver), typeSolver);
			}

			SignatureAttribute.ClassSignature classSignature = SignatureAttribute.toClassSignature(
				ctClass.getGenericSignature());
			return JavassistUtils
				.signatureTypeToType(classSignature.getSuperClass(), typeSolver, this)
				.asReferenceType();
		}
		catch (NotFoundException e)
		{
	...
	@Override public List<ReferenceType> getInterfaces()
	{
		try
		{
			if (ctClass.getGenericSignature() == null)
	...
	@Override public Set<ReferenceTypeDeclaration> internalTypes()
	{
		...
		{
			/*
			Get all internal types of the current class and get their corresponding
			ReferenceTypeDeclaration. Finally, return them in a Set.
			 */
			return Arrays.stream(ctClass.getDeclaredClasses())
				.map(itype -> JavassistFactory.toTypeDeclaration(itype, typeSolver))
	...
	@Override public ReferenceTypeDeclaration getInternalType(String name)
	{
		/*
		The name of the ReferenceTypeDeclaration could be composed of the internal class and the
		outer class, e.g. A$B. That's why we search the internal type in the ending part. In case
		the name is composed of the internal type only, i.e. f.getName() returns B, it will also
		works.
		 */
