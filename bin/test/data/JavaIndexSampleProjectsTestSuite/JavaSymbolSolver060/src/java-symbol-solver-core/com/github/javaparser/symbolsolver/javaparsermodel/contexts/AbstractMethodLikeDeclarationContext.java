 ...
 */
public abstract class AbstractMethodLikeDeclarationContext<T extends Node & NodeWithParameters<T> &
	...
	extends AbstractJavaParserContext<T>
{
	...
	public final SymbolReference<? extends ValueDeclaration> solveSymbol(String name, TypeSolver typeSolver)
	{
		for (Parameter parameter: wrappedNode.getParameters())
		{
			...
			SymbolReference<? extends ValueDeclaration> symbolReference =
	...
	@Override public final Optional<Type> solveGenericType(String name, TypeSolver typeSolver)
	{
	...
	@Override public final Optional<Value> solveSymbolAsValue(String name, TypeSolver typeSolver)
	{
		for (Parameter parameter: wrappedNode.getParameters())
		{
			SymbolDeclarator sb = JavaParserFactory.getSymbolDeclarator(parameter, typeSolver);
	...
	public final SymbolReference<TypeDeclaration> solveType(String name, TypeSolver typeSolver)
	{
		...
		// Local types
		List<com.github.javaparser.ast.body.TypeDeclaration> localTypes =
			wrappedNode.getChildNodesByType(com.github.javaparser.ast.body.TypeDeclaration.class);
		for (com.github.javaparser.ast.body.TypeDeclaration<?> localType: localTypes)
		{
			...
			else if (name.startsWith(String.format("%s.", localType.getName())))
			{
				return JavaParserFactory.getContext(localType, typeSolver)
					.solveType(name.substring(localType.getName().getId().length() + 1), typeSolver);
			}
		}
