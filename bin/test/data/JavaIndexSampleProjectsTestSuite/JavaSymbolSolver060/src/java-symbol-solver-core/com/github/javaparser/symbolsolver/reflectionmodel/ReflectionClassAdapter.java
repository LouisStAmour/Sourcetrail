...
class ReflectionClassAdapter
{
	...
	public ReflectionClassAdapter(
		Class<?> clazz, TypeSolver typeSolver, ReferenceTypeDeclaration typeDeclaration)
	{
		this.clazz = clazz;
	...
	public ReferenceTypeImpl getSuperClass()
	{
		...
		return new ReferenceTypeImpl(
			new ReflectionClassDeclaration(clazz.getSuperclass(), typeSolver), typeSolver);
	}
	...
	public List<ReferenceType> getAncestors()
	{
		...
		for (int i = 0; i < ancestors.size(); i++)
		{
			...
			if (ancestor.hasName() &&
				ancestor.getQualifiedName().equals(Object.class.getCanonicalName()))
			{
				ancestors.remove(i);
				i--;
			}
		...
		for (ReferenceType ancestor: typeDeclaration.getAllAncestors())
		{
			if (ancestor.getTypeDeclaration().hasField(name))
			{
				ReflectionFieldDeclaration reflectionFieldDeclaration =
					(ReflectionFieldDeclaration)ancestor.getTypeDeclaration().getField(name);
