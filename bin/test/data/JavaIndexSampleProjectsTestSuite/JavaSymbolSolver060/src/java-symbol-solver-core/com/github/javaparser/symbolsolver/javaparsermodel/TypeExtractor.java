...
public class TypeExtractor extends DefaultVisitorAdapter
{
	...
	@Override public Type visit(VariableDeclarator node, Boolean solveLambdas)
	{
		...
		{
			//                VariableDeclarationExpr parent = (VariableDeclarationExpr)
			//                getParentNode(node);
			return facade.convertToUsageVariableType(node);
		}
		else
	...
	@Override public Type visit(Parameter node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(ArrayAccessExpr node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(ArrayCreationExpr node, Boolean solveLambdas)
	{
		Type res = facade.convertToUsage(
	...
	@Override public Type visit(ArrayInitializerExpr node, Boolean solveLambdas)
	{
		throw new UnsupportedOperationException(node.getClass().getCanonicalName());
	}

	@Override public Type visit(AssignExpr node, Boolean solveLambdas)
	{
		return node.getTarget().accept(this, solveLambdas);
	}

	@Override public Type visit(BinaryExpr node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(CastExpr node, Boolean solveLambdas)
	{
		return facade.convertToUsage(node.getType(), JavaParserFactory.getContext(node, typeSolver));
	}

	@Override public Type visit(ClassExpr node, Boolean solveLambdas)
	{
		...
		return new ReferenceTypeImpl(
			new ReflectionClassDeclaration(Class.class, typeSolver),
			ImmutableList.of(jssType),
			typeSolver);
	}

	@Override public Type visit(ConditionalExpr node, Boolean solveLambdas)
	{
		return node.getThenExpr().accept(this, solveLambdas);
	}

	@Override public Type visit(EnclosedExpr node, Boolean solveLambdas)
	{
	...
	private Type solveDotExpressionType(ReferenceTypeDeclaration parentType, FieldAccessExpr node)
	{
		...
		{
			return new ReferenceTypeImpl(
				parentType.getInternalType(node.getName().getId()), typeSolver);
		}
		else
		{
	...
	@Override public Type visit(FieldAccessExpr node, Boolean solveLambdas)
	{
		...
		if (node.getScope() instanceof NameExpr || node.getScope() instanceof FieldAccessExpr)
		{
			...
			SymbolReference<TypeDeclaration> typeAccessedStatically =
				JavaParserFactory.getContext(node, typeSolver)
					.solveType(staticValue.toString(), typeSolver);
		...
		else if (node.getScope().toString().indexOf('.') > 0)
		{
			...
			SymbolReference<ReferenceTypeDeclaration> sr = typeSolver.tryToSolveType(
		...
		catch (UnsolvedSymbolException use)
		{
			...
			SymbolReference<ReferenceTypeDeclaration> sref = typeSolver.tryToSolveType(
				node.toString());
			if (sref.isSolved())
			{
	...
	@Override public Type visit(InstanceOfExpr node, Boolean solveLambdas)
	{
		return PrimitiveType.BOOLEAN;
	}

	@Override public Type visit(StringLiteralExpr node, Boolean solveLambdas)
	{
		return new ReferenceTypeImpl(
			new ReflectionTypeSolver().solveType("java.lang.String"), typeSolver);
	}

	@Override public Type visit(IntegerLiteralExpr node, Boolean solveLambdas)
	{
		return PrimitiveType.INT;
	}

	@Override public Type visit(LongLiteralExpr node, Boolean solveLambdas)
	{
		return PrimitiveType.LONG;
	}

	@Override public Type visit(CharLiteralExpr node, Boolean solveLambdas)
	{
		return PrimitiveType.CHAR;
	}

	@Override public Type visit(DoubleLiteralExpr node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(BooleanLiteralExpr node, Boolean solveLambdas)
	{
		return PrimitiveType.BOOLEAN;
	}

	@Override public Type visit(NullLiteralExpr node, Boolean solveLambdas)
	{
		return NullType.INSTANCE;
	}

	@Override public Type visit(MethodCallExpr node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(NameExpr node, Boolean solveLambdas)
	{
		logger.finest("getType on name expr " + node);
		Optional<Value> value =
			new SymbolSolver(typeSolver).solveSymbolAsValue(node.getName().getId(), node);
		if (!value.isPresent())
		{
	...
	@Override public Type visit(ObjectCreationExpr node, Boolean solveLambdas)
	{
		Type type = facade.convertToUsage(node.getType(), node);
		return type;
	}

	@Override public Type visit(ThisExpr node, Boolean solveLambdas)
	{
		...
		if (node.getClassExpr().isPresent())
		{
			...
			if (cu.isPresent())
			{
				...
				{
					return new ReferenceTypeImpl(
						facade.getTypeDeclaration(classByName.get()), typeSolver);
				}
			}
		}
		return new ReferenceTypeImpl(
			facade.getTypeDeclaration(facade.findContainingTypeDecl(node)), typeSolver);
	}

	@Override public Type visit(SuperExpr node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(UnaryExpr node, Boolean solveLambdas)
	{
		switch (node.getOperator())
		{
		case MINUS:
	...
	@Override public Type visit(VariableDeclarationExpr node, Boolean solveLambdas)
	{
	...
	@Override public Type visit(LambdaExpr node, Boolean solveLambdas)
	{
		if (getParentNode(node) instanceof MethodCallExpr)
		{
			...
			if (!refMethod.isSolved())
			{
				throw new UnsolvedSymbolException(
					getParentNode(node).toString(), callExpr.getName().getId());
			}
			logger.finest(
				"getType on lambda expr " + refMethod.getCorrespondingDeclaration().getName());
			if (solveLambdas)
			{
				...
				if (callExpr.getScope().isPresent())
				{
					...
					if (scope instanceof NameExpr)
					{
						...
						try
						{
							SymbolReference<TypeDeclaration> type =
								JavaParserFactory.getContext(nameExpr, typeSolver)
									.solveType(nameExpr.getName().getId(), typeSolver);
							if (type.isSolved())
							{
								staticCall = true;
							}
					...
					if (!staticCall)
					{
						...
						{
							result = scopeType.asReferenceType().useThisTypeParametersOnTheGivenType(
								result);
						}
					}
				}
				...
				// We should find out which is the functional method (e.g., apply) and replace the
				// params of the solveLambdas with it, to derive so the values. We should also
				// consider the value returned by the lambdas
				...
				if (functionalMethod.isPresent())
				{
					...
					InferenceContext funcInterfaceCtx = new InferenceContext(
						MyObjectProvider.INSTANCE);

					// At this point parameterType
					// if Function<T=? super Stream.T, ? extends map.R>
					// we should replace Stream.T
					Type functionalInterfaceType = ReferenceTypeImpl.undeterminedParameters(
						functionalMethod.get().getDeclaration().declaringType(), typeSolver);
					...
					{
						actualType = facade.getType(
							((ExpressionStmt)lambdaExpr.getBody()).getExpression());
					}
					...
					Type functionalTypeWithReturn = funcInterfaceCtx.resolve(
						funcInterfaceCtx.addSingle(functionalInterfaceType));
	...
	@Override public Type visit(MethodReferenceExpr node, Boolean solveLambdas)
	{
		if (getParentNode(node) instanceof MethodCallExpr)
		{
			MethodCallExpr callExpr = (MethodCallExpr)getParentNode(node);
			int pos = JavaParserSymbolDeclaration.getParamPos(node);
			SymbolReference<com.github.javaparser.symbolsolver.model.declarations.MethodDeclaration>
				refMethod = facade.solve(callExpr, false);
			if (!refMethod.isSolved())
			{
				throw new UnsolvedSymbolException(
					getParentNode(node).toString(), callExpr.getName().getId());
			...
			logger.finest(
				"getType on method reference expr " +
