...
public class MethodCallExprContext extends AbstractJavaParserContext<MethodCallExpr>
{
		...
		String name, List<Type> argumentsTypes, TypeSolver typeSolver)
	{
		if (wrappedNode.getScope().isPresent())
		{
			...
			if (scope instanceof NameExpr)
			{
				...
				if (ref.isSolved())
				{
					SymbolReference<MethodDeclaration> m = MethodResolutionLogic.solveMethodInType(
						ref.getCorrespondingDeclaration(), name, argumentsTypes, typeSolver);
					if (m.isSolved())
					{
						MethodUsage methodUsage = new MethodUsage(m.getCorrespondingDeclaration());
						methodUsage = resolveMethodTypeParametersFromExplicitList(
							typeSolver, methodUsage);
			...
			for (int i = 0; i < argumentsTypes.size(); i++)
			{
				...
				// for example if I replace T=U with String because I know that T=String I can
				// derive that also U equal String
				Type originalArgumentType = argumentsTypes.get(i);
				Type updatedArgumentType = usingParameterTypesFromScope(
					typeOfScope, originalArgumentType, inferredTypes);
		...
		TypeSolver typeSolver, MethodUsage methodUsage)
	{
		if (wrappedNode.getTypeArguments().isPresent())
		{
			...
			List<TypeParameterDeclaration> tyParamDecls =
			...
			if (tyParamDecls.size() == typeArguments.size())
			{
				for (int i = 0; i < tyParamDecls.size(); i++)
				{
					methodUsage = methodUsage.replaceTypeParameter(
						tyParamDecls.get(i), typeArguments.get(i));
				}
			}
		}
		...
		String name, List<Type> argumentsTypes, boolean staticOnly, TypeSolver typeSolver)
	{
		if (wrappedNode.getScope().isPresent())
		{
			...
			if (scope instanceof NameExpr)
			{
				...
				SymbolReference<TypeDeclaration> symbolReference = this.solveType(
					scopeAsName.getName().getId(), typeSolver);
				if (symbolReference.isSolved() &&
					symbolReference.getCorrespondingDeclaration().isType())
				{
					TypeDeclaration typeDeclaration =
						symbolReference.getCorrespondingDeclaration().asType();
					return MethodResolutionLogic.solveMethodInType(
						typeDeclaration, name, argumentsTypes, false, typeSolver);
				}
			}
			...
			catch (Exception e)
			{
				throw new RuntimeException(
					String.format("Issue calculating the type of the scope of " + this), e);
			}
			...
			else if (typeOfScope.isTypeVariable())
			{
				for (TypeParameterDeclaration.Bound bound:
					 typeOfScope.asTypeParameter().getBounds(typeSolver))
				{
			...
			{
				return MethodResolutionLogic.solveMethodInType(
					typeOfScope.asReferenceType().getTypeDeclaration(),
					name,
					argumentsTypes,
					false,
					typeSolver);
		...
		Context invokationContext)
	{
		...
		if (ref.isPresent())
		{
			...
			// At this stage I should derive from the context and the value some information on the
			// type parameters for example, when calling: myStream.collect(Collectors.toList()) I
			// should be able to figure out that considering the type of the stream (e.g.,
			// Stream<String>) and considering that Stream has this method:
			//
			...
			// In our example Stream.T equal to String, so the R (and the result of the call to
			// collect) is List<? super String>
			...
			for (int i = 0; i < methodUsage.getParamTypes().size(); i++)
			{
				Type replaced = refType.useThisTypeParametersOnTheGivenType(
	...
	private MethodUsage resolveMethodTypeParameters(MethodUsage methodUsage, List<Type> actualParamTypes)
	{
		...
		if (methodUsage.getDeclaration().hasVariadicParameter())
		{
			if (actualParamTypes.size() == methodUsage.getDeclaration().getNumberOfParams())
			{
				...
				Type actualType = actualParamTypes.get(actualParamTypes.size() - 1).isArray()
				...
				{
					for (TypeParameterDeclaration tp:
						 methodUsage.getDeclaration().getTypeParameters())
					{
						expectedType = MethodResolutionLogic.replaceTypeParam(
		...
		int until = methodUsage.getDeclaration().hasVariadicParameter() ? actualParamTypes.size() - 1
	...
	private void matchTypeParameters(
		Type expectedType, Type actualType, Map<TypeParameterDeclaration, Type> matchedTypeParameters)
	{
		...
		Context invokationContext)
	{
		...
		{
			Optional<MethodUsage> methodUsage = solveMethodAsUsage(
				bound.getType(), name, argumentsTypes, typeSolver, invokationContext);
	...
	private Optional<MethodUsage> solveMethodAsUsage(
		Type type, String name, List<Type> argumentsTypes, TypeSolver typeSolver, Context invokationContext)
	{
		if (type instanceof ReferenceType)
		{
			return solveMethodAsUsage(
				(ReferenceType)type, name, argumentsTypes, typeSolver, invokationContext);
		}
		else if (type instanceof TypeVariable)
		{
			return solveMethodAsUsage(
				(TypeVariable)type, name, argumentsTypes, typeSolver, invokationContext);
		}
		else if (type instanceof Wildcard)
		{
			...
			{
				return solveMethodAsUsage(
					wildcardUsage.getBoundedType(), name, argumentsTypes, typeSolver, invokationContext);
			}
		...
		else if (type instanceof LambdaConstraintType)
		{
			...
			return solveMethodAsUsage(
				constraintType.getBound(), name, argumentsTypes, typeSolver, invokationContext);
		}
	...
	private Type usingParameterTypesFromScope(
		Type scope, Type type, Map<TypeParameterDeclaration, Type> inferredTypes)
	{
		if (type.isReferenceType())
		{
			for (Tuple2<TypeParameterDeclaration, Type> entry:
				 type.asReferenceType().getTypeParametersMap())
			{
				if (entry._1.declaredOnType() &&
					scope.asReferenceType().getGenericParameterByName(entry._1.getName()).isPresent())
				{
					type = type.replaceTypeVariables(
						entry._1,
						scope.asReferenceType().getGenericParameterByName(entry._1.getName()).get(),
						inferredTypes);
				}
			}
			return type;
