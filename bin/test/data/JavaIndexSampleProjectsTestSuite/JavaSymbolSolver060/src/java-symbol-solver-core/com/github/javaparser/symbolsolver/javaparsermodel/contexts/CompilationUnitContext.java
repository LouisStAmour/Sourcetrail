...
public class CompilationUnitContext extends AbstractJavaParserContext<CompilationUnit>
{
	...
	public SymbolReference<? extends ValueDeclaration> solveSymbol(String name, TypeSolver typeSolver)
	{
		...
		while (itName.contains("."))
		{
			...
			SymbolReference<com.github.javaparser.symbolsolver.model.declarations.TypeDeclaration> type =
				this.solveType(typeName, typeSolver);
			if (type.isSolved())
			{
				...
				if (importDecl.isStatic())
				{
					if (importDecl.isAsterisk())
					{
						...
						com.github.javaparser.symbolsolver.model.declarations
							.TypeDeclaration importedType = typeSolver.solveType(qName);
					...
					else
					{
						...
						{
							com.github.javaparser.symbolsolver.model.declarations
								.TypeDeclaration importedType = typeSolver.solveType(typeName);
							return new SymbolSolver(typeSolver)
		...
		String name, TypeSolver typeSolver)
	{
				...
				if (type.getName().getId().equals(name))
				{
					...
					else if (type instanceof AnnotationDeclaration)
					{
						return SymbolReference.solved(new JavaParserAnnotationDeclaration(
							(AnnotationDeclaration)type, typeSolver));
					}
					else
					{
		...
		if (wrappedNode.getImports() != null)
		{
			...
			for (ImportDeclaration importDecl: wrappedNode.getImports())
			{
				if (!importDecl.isAsterisk())
				{
					...
					if (found)
					{
						SymbolReference<com.github.javaparser.symbolsolver.model.declarations
											.ReferenceTypeDeclaration> ref =
							typeSolver.tryToSolveType(qName);
						if (ref.isSolved())
						{
							return SymbolReference.adapt(
								ref,
								com.github.javaparser.symbolsolver.model.declarations
			...
			for (ImportDeclaration importDecl: wrappedNode.getImports())
			{
				if (importDecl.isAsterisk())
				{
					...
					if (ref.isSolved())
					{
						return SymbolReference.adapt(
							ref,
							com.github.javaparser.symbolsolver.model.declarations.TypeDeclaration.class);
					}
				}
			}
		}
		...
		{
			String qName = this.wrappedNode.getPackageDeclaration().get().getName().toString() +
				"." + name;
			SymbolReference<com.github.javaparser.symbolsolver.model.declarations.ReferenceTypeDeclaration>
				ref = typeSolver.tryToSolveType(qName);
			if (ref.isSolved())
			{
				return SymbolReference.adapt(
					ref, com.github.javaparser.symbolsolver.model.declarations.TypeDeclaration.class);
			}
		}
		else
		{
		...
		// DO NOT look for absolute name if this name is not qualified: you cannot import classes
		...
		else
		{
			return SymbolReference.unsolved(
				com.github.javaparser.symbolsolver.model.declarations.ReferenceTypeDeclaration.class);
		}
	}
		...
		for (ImportDeclaration importDecl: wrappedNode.getImports())
		{
			if (importDecl.isStatic())
			{
				if (importDecl.isAsterisk())
				{
					...
					if (this.wrappedNode.getPackageDeclaration().isPresent() &&
						this.wrappedNode.getPackageDeclaration().get().getName().getIdentifier().equals(
							packageName(importString)) &&
						this.wrappedNode.getTypes().stream().anyMatch(
							it -> it.getName().getIdentifier().equals(toSimpleName(importString))))
					{
						// We are using a static import on a type defined in this file. It means the
						// value was not found at a lower level so this will fail
						return SymbolReference.unsolved(MethodDeclaration.class);
					}

					com.github.javaparser.symbolsolver.model.declarations.TypeDeclaration ref =
						typeSolver.solveType(importString);
					SymbolReference<MethodDeclaration> method = MethodResolutionLogic.solveMethodInType(
						ref, name, argumentsTypes, true, typeSolver);
				...
				else
				{
					...
					if (qName.equals(name) || qName.endsWith("." + name))
					{
						...
						com.github.javaparser.symbolsolver.model.declarations.TypeDeclaration ref =
							typeSolver.solveType(typeName);
						SymbolReference<MethodDeclaration> method =
