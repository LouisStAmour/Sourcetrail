...
public class AnonymousClassDeclarationContext extends AbstractJavaParserContext<ObjectCreationExpr>
{
		...
		String name, List<Type> argumentsTypes, boolean staticOnly, TypeSolver typeSolver)
	{
		List<MethodDeclaration> candidateMethods =
			myDeclaration.getDeclaredMethods()
				.stream()
		...
		if (!Object.class.getCanonicalName().equals(myDeclaration.getQualifiedName()))
		{
			for (ReferenceType ancestor: myDeclaration.getAncestors())
			{
				SymbolReference<MethodDeclaration> res = MethodResolutionLogic.solveMethodInType(
					ancestor.getTypeDeclaration(), name, argumentsTypes, staticOnly, typeSolver);
		...
		{
			SymbolReference<MethodDeclaration> parentSolution = getParent().solveMethod(
	...
	@Override public SymbolReference<TypeDeclaration> solveType(String name, TypeSolver typeSolver)
	{
		List<com.github.javaparser.ast.body.TypeDeclaration> typeDeclarations =
			myDeclaration.findMembersOfKind(com.github.javaparser.ast.body.TypeDeclaration.class);
			...
			typeDeclarations.stream()
				...
				.map(
					...
					-> JavaParserFactory.getContext(internalType, typeSolver)
						   .solveType(
							   name.substring(internalType.getName().getId().length() + 1),
							   typeSolver));
		...
		Optional<SymbolReference<TypeDeclaration>> typeArgumentsMatch =
			wrappedNode.getTypeArguments()
				...
				.map(
					matchingType
					-> SymbolReference.solved(new JavaParserTypeParameter(
		...
		for (ReferenceType ancestor: myDeclaration.getAncestors())
		{
			// look at names of extended classes and implemented interfaces (this may not be
			// important because they are checked in CompilationUnitContext)
			...
			try
			{
				for (TypeDeclaration internalTypeDeclaration:
					 ancestor.getTypeDeclaration().internalTypes())
				{
