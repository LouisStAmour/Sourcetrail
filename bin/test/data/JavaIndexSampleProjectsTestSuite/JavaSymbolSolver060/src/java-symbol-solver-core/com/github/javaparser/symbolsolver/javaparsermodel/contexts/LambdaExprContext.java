...
public class LambdaExprContext extends AbstractJavaParserContext<LambdaExpr>
{
				...
				if (decl.getName().equals(name))
				{
					if (getParentNode(wrappedNode) instanceof MethodCallExpr)
					{
						...
						if (functionalMethodOpt.isPresent())
						{
							...
							// Resolve each type variable of the lambda, and use this later to infer
							// the type of each implicit parameter
							inferenceContext.addPair(
								lambdaType,
								new ReferenceTypeImpl(
									lambdaType.asReferenceType().getTypeDeclaration(), typeSolver));
							...
							for (lambdaParamIndex = 0;
								 ...
								 lambdaParamIndex++)
							{
								if (wrappedNode.getParameter(lambdaParamIndex)
										.getName()
										.getIdentifier()
										.equals(name))
								{
							...
							Type argType = inferenceContext.resolve(inferenceContext.addSingle(
								functionalMethod.getParamType(lambdaParamIndex)));

							LambdaConstraintType conType;
							if (argType.isWildcard())
							{
								conType = LambdaConstraintType.bound(
					...
					{
						VariableDeclarator variableDeclarator = (VariableDeclarator)getParentNode(
							wrappedNode);
						Type t = JavaParserFacade.get(typeSolver)
									 .convertToUsageVariableType(variableDeclarator);
						...
						if (functionalMethod.isPresent())
						{
							...
							if (lambdaType.isReferenceType())
							{
								...
								{
									if (entry._2.isTypeVariable() &&
										entry._2.asTypeParameter().declaredOnType())
									{
										Type ot = t.asReferenceType().typeParametersMap().getValue(
											entry._1);
										lambdaType = lambdaType.replaceTypeVariables(
							...
							{
								lambdaType = t.asReferenceType().typeParametersMap().getValue(
									lambdaType.asTypeParameter());
							}
	...
	protected final Optional<Value> solveWithAsValue(
