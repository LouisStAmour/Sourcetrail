...
/**
 * It prints information extracted from a source file. It is mainly intended as an example usage of
 * JavaSymbolSolver.
 *
...
public class SourceFileInfoExtractor
{
	...
	private void solve(Node node)
	{
		...
		{
			if ((getParentNode(node) instanceof ImportDeclaration) ||
				(getParentNode(node) instanceof Expression) ||
			...
			else if ((getParentNode(node) instanceof Statement) || (getParentNode(node) instanceof VariableDeclarator))
			{
				try
				{
					...
					out.println(
						"  Line " + node.getRange().get().begin.line + ") " + node + " ==> " +
						ref.describe());
					ok++;
				}
	...
	private void solveMethodCalls(Node node)
	{
		...
		{
			out.println(
				"  Line " + node.getBegin().get().line + ") " + node + " ==> " +
	...
	private String toString(MethodCallExpr node)
	{
		...
		catch (Exception e)
		{
			...
			{
				System.err.println(
					"Error resolving call at L" + node.getBegin().get().line + ": " + node);
				e.printStackTrace();
			}
			return "ERROR";
		}
	...
	private String toString(
		SymbolReference<com.github.javaparser.symbolsolver.model.declarations.MethodDeclaration>
			methodDeclarationSymbolReference)
	{
