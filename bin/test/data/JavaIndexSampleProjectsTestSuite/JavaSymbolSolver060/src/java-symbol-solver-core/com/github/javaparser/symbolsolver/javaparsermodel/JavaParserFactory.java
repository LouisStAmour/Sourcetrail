...
public class JavaParserFactory
{
	public static Context getContext(Node node, TypeSolver typeSolver)
	{
		...
		{
			return new ClassOrInterfaceDeclarationContext(
				(ClassOrInterfaceDeclaration)node, typeSolver);
		}
		...
		else
		{
			...
			{
				// to resolve a name when in a fieldAccess context, we can get to the grand parent
				// to prevent a infinite loop if the name is the same as the field (ie x.x)
	...
	public static SymbolDeclarator getSymbolDeclarator(Node node, TypeSolver typeSolver)
	{
		...
		else if (node instanceof ForeachStmt)
		{
			...
			return new VariableSymbolDeclarator(
				(VariableDeclarationExpr)(foreachStmt.getVariable()), typeSolver);
		}
		else
		{
