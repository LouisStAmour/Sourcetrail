...
public abstract class ReferenceType implements Type, TypeParametrized, TypeParameterValueProvider
{
	...
	public ReferenceType(
		ReferenceTypeDeclaration typeDeclaration, List<Type> typeParameters, TypeSolver typeSolver)
	{
		if (typeSolver == null)
		{
		...
		for (int i = 0; i < typeParameters.size(); i++)
		{
			typeParametersMapBuilder.setValue(
				typeDeclaration.getTypeParameters().get(i), typeParameters.get(i));
		}
	...
	@Override public ReferenceType asReferenceType()
	{
		return this;
	}
	 ...
	 */
	@Override public abstract boolean isAssignableBy(Type other);
	 ...
	 * It returns Optional.empty unless the type declaration declares a type parameter with the
	 * given name.
	 */
	...
	public List<Tuple2<TypeParameterDeclaration, Type>> getTypeParametersMap()
	{
		...
		if (!isRawType())
		{
			...
			{
				typeParametersMap.add(new Tuple2<>(
					typeDeclaration.getTypeParameters().get(0), typeParametersValues().get(i)));
			}
		}
		return typeParametersMap;
	}

	@Override public TypeParametersMap typeParametersMap()
	{
		return typeParametersMap;
	}
	...
	protected abstract ReferenceType create(
		ReferenceTypeDeclaration typeDeclaration, List<Type> typeParameters, TypeSolver typeSolver);

	protected ReferenceType create(
	...
	protected abstract ReferenceType
		create(ReferenceTypeDeclaration typeDeclaration, TypeSolver typeSolver);
			...
			for (int i = 0; i < typeParametersValues().size(); i++)
			{
				...
				if (!thisParam.equals(otherParam))
				{
					...
					else
					{
						...
						{
							List<Type> thisBounds = thisParam.asTypeVariable()
														.asTypeParameter()
														.getBounds(this.typeSolver)
														.stream()
														.map(bound -> bound.getType())
														.collect(Collectors.toList());
							List<Type> otherBounds = otherParam.asTypeVariable()
														 .asTypeParameter()
														 .getBounds(other.typeSolver)
														 .stream()
														 .map(bound -> bound.getType())
														 .collect(Collectors.toList());
							if (thisBounds.size() == otherBounds.size() &&
	...
	{
		return typeDeclaration.getTypeParameters()
			.stream()
			.map((tp) -> new TypeVariable(tp))
			.collect(Collectors.toList());
	}
