...
public class InferenceContext
{
	...
	private InferenceVariableType inferenceVariableTypeForTp(TypeParameterDeclaration tp)
	{
		if (!inferenceVariableTypeMap.containsKey(tp.getName()))
		{
			InferenceVariableType inferenceVariableType = new InferenceVariableType(
				nextInferenceVariableId++, objectProvider);
	...
	private void registerCorrespondance(Type formalType, Type actualType)
	{
		if (formalType.isReferenceType() && actualType.isReferenceType())
		{
			...
			if (!formalTypeAsReference.getQualifiedName().equals(
					actualTypeAsReference.getQualifiedName()))
			{
				...
				List<Type> correspondingFormalType =
					ancestors.stream()
				...
				if (correspondingFormalType.isEmpty())
				{
					...
					List<Type> correspondingActualType =
						ancestors.stream()
		...
		else if (actualType.isPrimitive())
		{
			...
			{
				registerCorrespondance(
					formalType, objectProvider.byName(actualType.asPrimitive().getBoxTypeQName()));
			}
		}
		else
		{
	...
	private Type placeInferenceVariables(Type type)
	{
		...
		{
			return LambdaConstraintType.bound(
