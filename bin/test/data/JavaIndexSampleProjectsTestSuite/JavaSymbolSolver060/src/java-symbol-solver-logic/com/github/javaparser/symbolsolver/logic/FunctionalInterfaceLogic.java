...
public final class FunctionalInterfaceLogic
{
	...
	public static Optional<MethodUsage> getFunctionalMethod(ReferenceTypeDeclaration typeDeclaration)
	{
		...
		Set<MethodUsage> methods = typeDeclaration.getAllMethods()
									   .stream()
									   .filter(m -> m.getDeclaration().isAbstract())
									   // Remove methods inherited by Object:
									   // Consider the case of Comparator which define equals. It
									   // would be considered a functional method.
	...
	private static String getSignature(Method m)
	{
		return String.format(
			...
			String.join(
				", ",
				Arrays.stream(m.getParameters()).map(p -> toSignature(p)).collect(Collectors.toList())));
	}
